...
def make_json_serializable(d):
    ...
    for k, v in d.items():
        ...
        # For lists, recursively
            ...
            out[k] = [make_json_serializable(x) if isinstance(x, dict) else str(x) for x in v]
...
# === DATA LOADING ===
TRAIN_PATH = f"{Path(__file__).parent}/data/cleaned_datasets/dataset_training_benign.csv"
TEST_PATH = f"{Path(__file__).parent}/data/cleaned_datasets/dataset_3_cleaned.csv"
...
BEST_PARAMS_PATH = "detector_best_params.json"
...
for model_name, param_grid in param_grid_models.items():
    ...
    if os.path.exists(model_file):
        ...
        if model_name in all_best_params:
            ...
            pred_test = best_detector.predict(X_test)
...
results_df.to_csv("detector_results_gridsearch.csv", index=False)
print("\nAll grid search results saved to detector_results_gridsearch.csv")
