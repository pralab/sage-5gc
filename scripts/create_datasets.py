...
if __name__ == "__main__":
    ...
    df_test_filtered[cat_cols] = simple_imputer.transform(df_test_processed[cat_cols])
    df_test_filtered[num_cols] = iter_imputer.transform(df_test_processed[num_cols])
    ...
    df_train_filtered = restore_categoric_columns(df_train_filtered, cat_cols_train)
    df_test_filtered = restore_categoric_columns(df_test_filtered, cat_cols_test)

    df_train_filtered["ip.opt.time_stamp"] = labels_train
    df_test_filtered["ip.opt.time_stamp"] = labels_test

    df_train_filtered.to_csv(dataset_dir / "train_dataset2.csv", sep=";", index=False)
    df_test_filtered.to_csv(dataset_dir / "test_dataset2.csv", sep=";", index=False)
