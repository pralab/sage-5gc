"""Script to create a heatmap visualization of class-wise performance of models."""
...
RESULTS_DIR = (
    Path(__file__).parent.parent / "results/with_scaler/category_results"
)
...
def load_model_results(json_path: Path):
    ...
    for attack_key, display_name in ATTACK_CATEGORIES.items():
        row[display_name] = attacks.get(attack_key, {}).get(
...
def plot_heatmap(df: pd.DataFrame):
    ...
    plt.title(
        "Class-wise Performance Heatmap (Normal + Attacks)", fontsize=18, pad=20
    )
    ...
    out_path = FIGURES_DIR / "heatmap_detection_rate_scaler.pdf"
