"""Pipeline to train and evaluate multiple anomaly detection models using Grid Search."""
...
def _make_json_serializable(d: dict) -> dict:
    ...
    for k, v in d.items():
        ...
        if hasattr(v, "get_params"):
            ...
            out[k] = [
                _make_json_serializable(x) if isinstance(x, dict) else str(x)
                for x in v
            ]
    ...
    for model_name, param_grid in PARAM_GRID_MODELS.items():
        ...
        if best_params is None:
            ...
            with BEST_PARAMS_PATH.open("w") as f:
                serializable = {
                    m: _make_json_serializable(p)
                    for m, p in all_best_params.items()
                }
