...
class EnsembleDetector(BaseDetector):
    ...
    def _meta_features_raw(self, X: pd.DataFrame, skip_preprocess: bool) -> np.ndarray:
        scores = []
    ...
    def _meta_features(self, X: pd.DataFrame, skip_preprocess: bool) -> np.ndarray:
        S = self._meta_features_raw(X, skip_preprocess)
        return (S - self._meta_mean) / self._meta_std

    def fit(self, X: pd.DataFrame, y: np.ndarray, skip_preprocess: bool = False) -> None:
        S = self._meta_features_raw(X, skip_preprocess)
    ...
    def decision_function(self, X: pd.DataFrame, skip_preprocess: bool = False) -> np.ndarray:
        X_meta = self._meta_features(X, skip_preprocess)
