...
def evaluate_predictions_if(y_true, y_pred):
    ...
    logger.info(classification_report(y_true, y_pred, digits=4, labels=[-1, 1], zero_division=0))
...
def run_isolation_forest(detector: DetectionIsolationForest, test_csv: str, save_plot: bool = True,
    ...
    logger.info(f"Macro Precision: {macro_precision_list[best_index]:.6f}, "
    ...
    logger.info(f"Precision: {precision_raw[best_index]:.6f}, Recall: {recall_raw[best_index]:.6f}, F1: {f1_scores_raw[best_index]:.6f}")
    ...
    for i, class_name in enumerate(class_names):
        if class_name != "Miss":
            ...
            logger.info(f"Precision: {prec * 100:.2f}% | Recall: {rec * 100:.2f}% | F1: {f1 * 100:.2f}%")
