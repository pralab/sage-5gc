...
class DetectionAutoEncoder:
    ...
    def optimize_threshold(self, enforce_percentile=None):
        ...
        if enforce_percentile is not None:
            ...
            print(f"Forced threshold (percentile {enforce_percentile}): {self.threshold:.6f} (F1={f1:.4f})")
        else:
            ...
            print(f"Chosen threshold (percentile {best['q']}): {self.threshold}")
    ...
    def run_predict(self, df_test: pd.DataFrame) -> tuple:
        ...
        if self.threshold is None:
            raise ValueError(
                "Threshold not set! "
                "Execute run_autoencoder() with optimize_threshold() "
                "before using run_predict() for robustness evaluation."
            )
