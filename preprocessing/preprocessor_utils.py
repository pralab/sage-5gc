...
def compute_pairwise_correlations(
    ...
    logger.debug(f"Loading sample {file_path}...")
    df = pd.read_csv(
...
def find_correlated_pairs(corr, cols, input_path, threshold=0.90):
    """
        ...
        for j in range(i + 1, len(cols)):
            ...
            if c is not None and abs(c) >= threshold:
                ...
                if var1 < var2:
                    ...
                    logger.debug(
                        f"Delete {col1} (corr label {var1:.3f}) vs {col2} (corr label {var2:.3f})"
                    )
                else:
                    cols_to_remove.add(col2)
                    logger.debug(
                        f"Delete {col2} (corr label {var2:.3f}) vs {col1} (corr label {var1:.3f})"
                    )

    logger.debug(
        f"{len(correlated_pairs)} correlated pairs detected (|corr| >= {threshold})"
    )
    logger.debug(f"Columns to remove: {sorted(cols_to_remove)}")
...
def compute_pearson_filter_multi(input_path, output_path):
    ...
    logger.debug(f"Common numeric cols across datasets: {len(cols)}")
    # Identify globally removable columns
    cols_to_remove = find_correlated_pairs(corr, cols, input_path, threshold=0.90)
    logger.debug("Columns removed:")
    for col in sorted(cols_to_remove):
        logger.debug(f" - {col}")
    # Save removal list
    with (Path(__file__).parent / "models_preprocessing/cols_to_remove.json").open(
        "w"
    ) as f:
...
def preprocessing_pipeline_train(
    ...
    """
    Apply the preprocessing pipeline for the train process.
    Can run from a DataFrame or from a CSV path.
    ...
    enrich_tcp_columns(df_train, tcp_path)
    ...
    (Path(__file__).parent / "models_preprocessing").mkdir(parents=True, exist_ok=True)
    dump(
        saine_imputer, Path(__file__).parent / "models_preprocessing/imputer_saine.pkl"
    )
    with (Path(__file__).parent / "models_preprocessing/saine_cols.json").open(
        "w"
    ) as f:
    ...
    for col in list(non_num_cols):
        try:
            ...
            pass
    if fake_num_cols:
        logger.debug(f"Recast object -> numeric: {fake_num_cols}")
        with (Path(__file__).parent / "models_preprocessing/fake_num_cols.json").open(
            "w"
        ) as f:
    ...
    dump(encoder, Path(__file__).parent / "models_preprocessing/encoder.pkl")
    with (Path(__file__).parent / "models_preprocessing/non_num_cols.json").open(
        "w"
    ) as f:
    ...
    df = df[
        [col for col in df.columns if col not in freq_cols and col not in time_columns]
    ]
    ...
    df.to_csv("tmp/df_main.csv", sep=";", index=False)
    df_freq_cols.to_csv("tmp/df_freq.csv", sep=";", index=False)
    df_encoded.to_csv("tmp/df_encoded.csv", sep=";", index=False)
    df_time_columns.to_csv("tmp/df_time_columns.csv", sep=";", index=False)
    ...
    with open(encoded_path, "w") as f_out:
        for parts in zip(
            pd.read_csv("tmp/df_main.csv", sep=";", chunksize=chunk_size),
            pd.read_csv("tmp/df_freq.csv", sep=";", chunksize=chunk_size),
            pd.read_csv("tmp/df_encoded.csv", sep=";", chunksize=chunk_size),
            pd.read_csv("tmp/df_time_columns.csv", sep=";", chunksize=chunk_size),
        ):
    ...
    filtered_path = os.path.join(output_dir, "dataset_train_filtered.csv")
    ...
    with (
        Path(__file__).parent / "models_preprocessing/common_filtered_cols.json"
    ).open("w") as f:
    ...
    dump(scaler, Path(__file__).parent / "models_preprocessing/scaler.pkl")
    with (Path(__file__).parent / "models_preprocessing/columns_to_scale.json").open(
        "w"
    ) as f:
        json.dump(columns_to_scale, f, indent=2)

    final_path = os.path.join(output_dir, "dataset_train_final.csv")
...
def preprocessing_pipeline_test(
    ...
    """
    Apply the processing pipeline for the test process.
    Can run from a DataFrame or from a CSV path.
    ...
    # === STEP 3: ADVANCED CLEANING ===
    drop_path = os.path.join(output_dir, "dataset_test_drop.csv")
    drop_columns_chunked(tcp_path, drop_path, is_attack=True)

    # === STEP 4: IMPUTE NUMERICAL ===
    imputer = joblib.load(
        Path(__file__).parent / "models_preprocessing/imputer_saine.pkl"
    )
    with open(Path(__file__).parent / "models_preprocessing/saine_cols.json") as f:
        valid_cols = json.load(f)
    imputed_path = os.path.join(output_dir, "dataset_test_imputed.csv")
    ...
    with (
        Path(__file__).parent / "models_preprocessing/fake_num_cols.json"
    ).open() as f:
    ...
    encoded_path = os.path.join(output_dir, "dataset_test_encoded.csv")
    ...
    filtered_path = os.path.join(output_dir, "dataset_test_filtered.csv")
    ...
    final_path = os.path.join(output_dir, "dataset_test_final.csv")
    transform_file_with_scaler(
        filtered_path,
        final_path,
        scaler,
