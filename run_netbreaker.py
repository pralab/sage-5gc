...
from netbreaker import blackbox_attack
from proto_features_mapping import get_protocol_feature_mapping

# Configurazione
TEST_CSV = "preprocessing/cleaned_datasets/dataset_3_cleaned.csv"
MODELS_DIR = "trained_models"
OUTPUT_DIR = "netbreaker_results"
BUDGET = 100          # numero di valutazioni per l’ottimizzatore
NOISE_LEVEL = 0.1     # frazione di righe da perturbarare per feature
...
def safe_read_csv(path: str) -> pd.DataFrame:
    """Tenta di leggere il CSV in UTF‑8 e, in caso di errore, in Latin‑1."""
    try:
...
def load_models() -> Dict[str, object]:
    """Istanzia e carica ciascun modello di rilevamento."""
    models = {}
    # Isolation Forest
    try:
...
def main() -> None:
    """Esegue l’attacco black‑box su tutti i modelli."""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    logger.info(f"Loading dataset: {TEST_CSV}")
    ...
    for model_name, detector in models.items():
        ...
        logger.info(f"Best improvement for {model_name}: {improvement:.2f} percentage points")
        results[model_name] = {
            "best_params": best_params,
            "improvement": improvement,
        }

        # salva il dataset perturbato
        perturbed_path = Path(OUTPUT_DIR) / f"{optimizer_type}/perturbed_{model_name}.csv"
        try:
            perturbed_df.to_csv(perturbed_path, sep=";", index=False, encoding="latin-1")
        ...
        # salva i risultati per modello in JSON
        result_json_path = Path(OUTPUT_DIR) / f"{optimizer_type}/result_{model_name}.json"
        try:
